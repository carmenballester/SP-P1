...
vector<vector<Point2f>> corners(vector<Mat> images) {
    ...
    for(size_t i=0; i<images.size(); i++) {
        bool patternfound = findChessboardCorners(images[i], patternSize, corners[i],
             CALIB_CB_ADAPTIVE_THRESH + /*CALIB_CB_NORMALIZE_IMAGE + */CALIB_CB_FAST_CHECK);

        // Si se detecta, afinar la detección de las coordenadas de los píxeles de las esquinas
        if(patternfound) {
            cvtColor(images[i], imageGrey, CV_BGR2GRAY);
            cornerSubPix(imageGrey, corners[i], Size(10, 10), Size(-1, -1),
                         TermCriteria(CV_TERMCRIT_EPS /*+ CV_TERMCRIT_ITER*/, 30, 0.6));
            // Dibujar las esquinas detectadas del patrón en la imagen
            drawChessboardCorners(images[i], patternSize, corners[i], patternfound);
