...
vector<Mat> loadImages() {
    ...
    String folder = "imagesACircle/*.jpg";
...
vector<vector<Point2f>> corners(vector<Mat> images) {
    ...
    for(size_t i=0; i<images.size(); i++) {
        bool patternfound = findCirclesGrid(images[i], patternSizeAC, corners[i], CALIB_CB_ASYMMETRIC_GRID);

        // Si se detecta, afinar la detección de las coordenadas de los píxeles de las esquinas
        if(patternfound) {
            // Dibujar las esquinas detectadas del patrón en la imagen
            drawChessboardCorners(images[i], patternSizeAC, corners[i], patternfound);
...
vector<vector<Point3f>> calculatePoints(vector<Mat> images) {
    ...
    for(int i=0; i<patternSizeAC.height; i++) {
        for(int j=0; j<patternSizeAC.width; j++) {
            points.push_back(Point3f(float((2*j + i % 2)*circleASize), float(i*circleASize), 0));
        }
    }
